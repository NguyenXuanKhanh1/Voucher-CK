...
namespace ChargeHub.Resource
{
    ...
    public class SuccessMessageFactory : SuccessMessages
    {
        public static string? GetNamedMessage(string name)
        {
            return ResourceManager.GetString(name, Thread.CurrentThread.CurrentUICulture) ?? ErrorMessages.GeneralNotDefinedError;
        }

        public static string GetNamedMessage(string name, CultureInfo? culture)
        {
            return ResourceManager.GetString(name, culture ?? Culture) ?? ErrorMessageFactory.GetNamedMessage("GeneralNotDefinedError") ?? ErrorMessages.UNDEFINED_MESSAGE;
        }

        public static string GetNamedMessage(Enum code)
        {
            return ResourceManager.GetString(code.ToString(), Thread.CurrentThread.CurrentUICulture) ?? ErrorMessageFactory.GetNamedMessage("GeneralNotDefinedError") ?? ErrorMessages.UNDEFINED_MESSAGE;
        }

        public static string GetNamedMessage(Enum code, CultureInfo? culture)
        {
            return ResourceManager.GetString(code.ToString(), Culture) ?? ErrorMessageFactory.GetNamedMessage("GeneralNotDefinedError") ?? ErrorMessages.UNDEFINED_MESSAGE;
        }
    }
}
